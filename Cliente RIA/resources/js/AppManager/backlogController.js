    ...
    function backlogController($scope, backlogRepository, $timeout, $rootScope, factoryBacklog, syncRepository) {
        ...
        function init() {
            backlogRepository.open().then(function () {
                if (navigator.onLine) {
                    backlogRepository.cleanBacklog();
                    factoryBacklog.listar().then(function (sucesso) {
                        angular.forEach(sucesso.data, function (value, key) {
                            salvarBdLocal(value);
                        });
                        listAll();
                    }, function (error) {
        ...
        function  registerSse(){
            factoryBacklog.eventSource().onmessage = function (event) {
                //Codigo do o que fazer quando tiver mudança no servitor
                //Objeto com os dados
                var value = angular.fromJson(event.data);
                var item = value.data;
                switch (value.operacao) {
                    case "SAVE":
                        ...
                        break;
                    case "DELETE":
                        backlogRepository.getById(item).then(function (sucess) {
                            if (sucess != undefined) remove(sucess);
                        });
                        break;
                    case "UPDATE":
                        salvarBdLocal(item);
        ...
        function salvarBdLocal(item) {
            backlogRepository.saveBacklog(item).then(
                function () {
                    listAll();
                },
                function (erro) {
